...
#LAST_RULE=$($run show firewall name $AUTH_RULESET | grep -o 'rule [0-9]\+ {' | sort | tail -n 1 | grep -o '[0-9]\+')
LAST_RULE=$($run show firewall name $AUTH_RULESET | grep -o 'rule [0-9]\+ {' | cut -f2 -d' ' | sort -g | tail -n 3 | head -n 1)
NEXT_RULE=$(($LAST_RULE+1))

echo "Last Rule: $LAST_RULE Next Rule: $NEXT_RULE"
